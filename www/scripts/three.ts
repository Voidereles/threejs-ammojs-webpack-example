...
export class PhysicsTest extends Scene3D {
  ...
  async create() {
    ...
    new GLTFLoader().loadAsync("/models/bag.glb").then((gltf) => {
      const bag: any = gltf.scene.children[0];
      bag.position.set(1.5, 2, 1.5);
      bag.scale.set(2, 2, 2);
      this.scene.add(bag);
      this.physics.add.existing(bag, { shape: "convex" });
